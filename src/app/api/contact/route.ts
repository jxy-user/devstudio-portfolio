...
export async function POST(request: NextRequest) {
  try {
    ...
    if (pass) {
      // Dynamic import nodemailer only when password is set
      ...
      console.log("Set GMAIL_APP_PASSWORD env var on Vercel to enable email delivery.");
    }

    return NextResponse.redirect(
      new URL(`/?sent=1${!pass ? "&mode=log" : ""}`, request.url)
    );
  } catch (error) {
